...
- Digital Garden
	- Ein echt tolles Plugin (das allerdings wirklich einigen Sachverstand erfordert, um korrekt konfiguriert zu werden - es sind dazu auch einige Accounts bei Webdiensten wie z. B. GitHub, Vercel oder Netlify nötig), das einen "digitalen Garten" anlegt. Das ist im Grunde eine Website, auf der ich mit einem einzigen Befehl hübsch formatiert meine Notizen veröffentlichen kann - z. B. diesen Beitrag. Es ist also ein Blog, nur ohne die Möglichkeit der Interaktion mit den Nutzern.
- Excalidraw
	- Mein all-time favorite und must have (um zwei Anglizismen zu bemühen) Plugin. Ohne Excalidraw wäre Obsidian für mich nicht sinnvoll nutzbar. Es stellt die "unendlichen" Schreib- und Zeichenfläche bereit (wie oben schon erwähnt gibt es Excalidraw auch als [WebApp](https://excalidraw.com) mit einer tollen Kollaborationsfunktion), verwendet ebenfalls zum Speichern offene Standards und kann durch heruntergeladene oder eigene Scripte erweitert werden (mein Script, um visuell ansprechende Tabellen generieren zu können habe ich oben ja schon gezeigt). Obwohl nicht alle Funktionen so "smooth" funktionieren wie z. B. unter OneNote (insbesondere die Textbearbeitung ist eher rudimentär) bevorzuge ich Excalidraw mittlerweile, schon alleine aufgrund des speziellen "Looks" den die App den mit ihr erstellten Inhalten verleiht. 
- Full Calendar
